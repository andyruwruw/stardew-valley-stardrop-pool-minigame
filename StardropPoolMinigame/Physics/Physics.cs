...
namespace StardropPoolMinigame.Behaviors.Physics
{
	...
	internal abstract class Physics : IPhysics
	{
		...
		protected virtual void Bounce(EntityPhysics entity1, EntityPhysics entity2, bool offsetOnly = false)
		{
			...
			if (!offsetOnly)
			{
				var momentum = GetMomentum(entity1, entity2, angle);

				var ball1ResultingVelocity = Vector2.Subtract(
					entity1.GetVelocity(),
					Vector2.Multiply(
						angle,
						new Vector2(
							momentum * entity2.GetMass(),
							momentum * entity2.GetMass())));
				var ball2ResultingVelocity = Vector2.Add(
					entity2.GetVelocity(),
					Vector2.Multiply(
						angle,
						new Vector2(
							momentum * entity1.GetMass(),
							momentum * entity1.GetMass())));
		...
		protected virtual void ApplyFriction(EntityPhysics entity)
		{
			if (VectorHelper.GetMagnitude(entity.GetVelocity()) < GameConstants.Ball.MinimumVelocity &&
				VectorHelper.GetMagnitude(entity.GetVelocity()) > 0)
			{
				entity.SetVelocity(Vector2.Zero);
			}
			else
			{
				var friction = Vector2.Multiply(entity.GetVelocity(), GameConstants.Ball.FrictionAcceleration);

				// Friction ramps up the slower the ball gets
				if (VectorHelper.GetMagnitude(entity.GetVelocity()) <= GameConstants.Ball.HaltBeginVelocity &&
					VectorHelper.GetMagnitude(entity.GetVelocity()) > 0)
				{
					var direction = Vector2.Normalize(entity.GetVelocity());
					var percentShortStop =
						(GameConstants.Ball.HaltBeginVelocity - VectorHelper.GetMagnitude(entity.GetVelocity())) /
						GameConstants.Ball.HaltBeginVelocity;
					friction = Vector2.Add(friction,
						Vector2.Multiply(direction, GameConstants.Ball.HaltAcceleration * percentShortStop));
				}

				var minimum = Vector2.Min(Vector2.Zero, Vector2.Multiply(entity.GetVelocity(), -1));
				var maximum = Vector2.Max(Vector2.Zero, Vector2.Multiply(entity.GetVelocity(), -1));

				Vector2.Clamp(friction, minimum, maximum);

				entity.SetAcceleration(Vector2.Add(entity.GetAcceleration(), friction));
			}
		}
